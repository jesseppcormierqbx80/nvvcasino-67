...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-fda93cbb";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-fda93cbb">'+'<span class="tile-face-fda93cbb tile-front-fda93cbb"></span>'+'<span class="tile-face-fda93cbb tile-back-fda93cbb"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-fda93cbb")){return;}
var back=tile.querySelector(".tile-back-fda93cbb");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-fda93cbb");if(isMine){tile.classList.add("boom-fda93cbb");}
if(dim){tile.classList.add("dimmed-fda93cbb");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-fda93cbb")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-fda93cbb");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-fda93cbb")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-fda93cbb");void panelEl.offsetWidth;panelEl.classList.add("jolt-fda93cbb");}
endRound();}
